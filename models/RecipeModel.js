...
class RecipeModel {
    ...
    static async createRecipes(title, ingredient, type) {
        try {
            const [result] = await db.query("INSERT INTO recipes(title, ingredient, type) VALUES (?, ?, ?)",
    ...
    static async updateRecipes(id, title, ingredient, type) {
        try {
            const [result] = await db.query("UPDATE recipes SET title=?, ingredient=?, type=? WHERE id=?",
